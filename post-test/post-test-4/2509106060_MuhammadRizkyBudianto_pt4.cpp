...
void proses(jadwal *arr) {
    ...
    for (int i = 0; i < Jumlahkereta; i++) {
        if ((arr + i)->nomor == nomor) {
            cout << "| Penumpang : " << setw(40) << left << temp->nama << "|\n";
...
void Antrian(jadwal *arr) {
    ...
    while (temp != NULL) {
        for (int j = 0; j < Jumlahkereta; j++) {
            if (temp->nomor == (arr + j)->nomor) {
                cout << "| " << setw(2) << no++ << " ";
                cout << "| " << setw(15) << temp->nama;
...
void peek(jadwal *arr) {
    ...
    if (!isEmptyQ()) {
        for (int j = 0; j < Jumlahkereta; j++) {
            if (front->nomor == (arr + j)->nomor) {
                cout << "| Antrian Terdepan                                      |\n";
                cout << "| Nama : " << setw(42) << left << front->nama << "|\n";
